...
- `16-new-features.md`: Updated title to v1.0.88; added TOC entry; added a "New in v1.0.88" section covering OSC 777 notifications, Sessions tab dismiss confirmation, `/allow-all`/exact-path approvals surviving managed-settings failures, custom-agent reasoning-effort/startup reliability, `/fork` during active turns, and a Notable Fixes list (bottom-anchored dialog text selection, `ask_user` newline-on-Enter, sandboxed proxy-tunnel-failure bypass guidance, deferred MCP tool naming/listing fixes, prompt-mode background-task timeout warning, MCP permission-prompt resume stall fix, MCP transient-failure recovery, hook `cwd` fix, hook `additionalContext` combination, ACP/AHP/`--server` managed settings, GitHub MCP OAuth callback fix, `--plugin-dir` agents in server-mode sessions, cached-MCP-tool scoping, session-resume pending-event preservation, namespaced/ignored skills, MCP/plugin display names, bounded large-session resume memory, Connectors reauthorization prompt, indexed search glob filtering and `--files` ripgrep fallback)
- `08-advanced-features.md`: Added v1.0.88+ notes under GitHub MCP Server (Built-in) for the OAuth `/callback` scope-escalation fix; Custom Agents for the model-list-load-failure fix and `--plugin-dir` server-mode agent visibility; Setting Reasoning Effort for reasoning-effort applying on agent selection; the MCP reliability-fixes callout for transient-failure recovery, cached-tool scoping, deferred-tool naming/listing, and MCP/plugin display names; ACP Client MCP Servers for enterprise managed settings applying to ACP/AHP/`--server` sessions; Session Persistence → Resuming Sessions for the MCP-permission-prompt stall fix, pending-event preservation on save failure, and bounded large-session resume memory; HTTP Hooks for the hook `cwd` fix and combined `additionalContext`; Security Best Practices for the sandboxed proxy-tunnel bypass-guidance fix
- `04-slash-commands.md`: Added a v1.0.88+ note to `/allow-all` for surviving managed-settings refresh failures and exact-path approvals visible in `/list-dirs`/cleared by `/reset-allowed-tools`; added a v1.0.88+ note to `/fork` for running during active turns
- `03-interactive-features.md`: Added a v1.0.88+ note under Progress Indicators for OSC 777 terminal notifications; added a v1.0.88+ note under Error Handling for bottom-anchored dialog text selection and `ask_user` newline-on-Enter; added a v1.0.88+ note under Switching Sessions for the Sessions tab dismiss-confirmation behavior
- `07-github-integration.md`: Added a v1.0.88+ note under Authentication Issues for the Connectors reauthorization prompt
- `05-file-context.md`: Added a v1.0.88+ note under Finding Code for indexed search glob filtering and `--files` ripgrep-fallback accuracy
- `14-skills-system.md`: Added a v1.0.88+ note under Reloading Skills for namespaced custom skills and ignored skill directories
- `20-cicd-automation.md`: Added a v1.0.88+ note under the `--output-format json` flag for prompt mode's background-task timeout warning

### Feature Summary (v1.0.88)
- **New:** Optional OSC 777 terminal notifications for direct Ghostty/WezTerm sessions
- **New:** Sessions tab requires `x` then `x` again to confirm dismissal, distinguishing permanent local-session deletion from closing a server-backed session
- **New:** `/allow-all` and exact per-path session approvals survive a failed managed-settings refresh
- **New:** A custom agent's `reasoning-effort` applies as soon as the agent is selected, not only its model
- **New:** `/fork` can run during an active turn
- **Fixed:** Text selection in bottom-anchored dialogs (including login device codes); `ask_user` newline-on-Enter with `Ctrl+Enter`/`Ctrl+S` submit; sandboxed proxy-tunnel-failure bypass guidance; custom-agent startup model-list-load-failure false warnings; deferred MCP tool sanitized-name listing and no-resolvable-server-name listing; prompt-mode background-task timeout warning; MCP-permission-prompt resume stall; MCP transient-failure recovery; hook `cwd` defaulting to project root; hook `additionalContext` combination across session/subagent-start hooks; enterprise managed settings on ACP/AHP/`--server` sessions; GitHub MCP OAuth `/callback` scope-escalation fix; `--plugin-dir` agents visible in server-mode sessions; cached-MCP-tool address/header scoping; session-resume pending-event preservation on save failure; namespaced/ignored-directory skill discovery; MCP/plugin display names and descriptions; bounded transcript memory when resuming large local sessions; GitHub Connectors reauthorization prompt; indexed search glob filtering and `--files` ripgrep-fallback accuracy

## 2026-09-21 — Docs updated for v1.0.87

- `README.md`: Bumped version note to v1.0.87; added "Latest features" bullets for v1.0.87 (Auto routing tier user/managed startup defaults and strict/user-overridable org policy, steering-prompt consolidation and recall, `worktreePathTemplate` setting, sandbox proxies on Windows and proxy username/password auth, MCP reliability fixes, `slowConnectionThresholdMs`, rubber-duck agent for every model family)
- `16-new-features.md`: Updated title to v1.0.87; added TOC entry; added a "New in v1.0.87" section covering the Auto routing tier defaults/policy, steering-prompt consolidation and recall, `worktreePathTemplate`, sandbox proxies on Windows, and a Notable Fixes list (question-dialog number-key selection past 9, `/keep-alive` failure reporting, prompt-mode child-task recovery, large-session resume reliability, `--yolo` persistence, empty `strictKnownMarketplaces` hiding built-ins, `/help`/`/mcp show` mouse-selection highlighting, git-status process teardown, MCP auth-status accuracy, PR badge/GitHub tabs after refresh, MCP server isolation, `copilot mcp list/get` built-in server reporting, MCP list-change subscription connect fix, subagent tool-request approval dedup, secrets no longer logged, managed plugin marketplace policy loading, MCP reconnect resume fix, `slowConnectionThresholdMs`, live subagent timeline elapsed time, rubber-duck agent for every model family, terminal repaint allocation reduction)
- `22-models-and-costs.md`: Added a v1.0.87+ note to the Auto model row for startup defaults and org policy
- `21-team-setup.md`: Added an "Auto Routing Tier Defaults and Policy (v1.0.87+)" subsection under Advanced: Organization-Level Setup
- `03-interactive-features.md`: Added a v1.0.87+ note under Queued Messages for steering-prompt consolidation and recall; added a v1.0.87+ note under Error Handling for the question-dialog number-key selection fix; added a v1.0.87+ note under Timeline Commands for live subagent elapsed time
- `15-copilot-directory.md`: Added the `worktreePathTemplate` setting to the `settings.json` example schema
- `04-slash-commands.md`: Added a v1.0.87+ note to `/worktree` for `worktreePathTemplate`; added a v1.0.87+ note to `/keep-alive` for the failure-reporting fix; added a v1.0.87+ note to `/rubber-duck` for enablement across every model family; added a v1.0.87+ note under `/mcp` for `copilot mcp list/get` built-in server reporting and MCP reliability fixes
- `08-advanced-features.md`: Added a v1.0.87+ note under Security Best Practices for sandbox proxies on Windows and username/password proxy auth; added a v1.0.87+ note under Plugin Marketplaces for the empty `strictKnownMarketplaces` fix; added a new "`slowConnectionThresholdMs` — Per-Server Slow-Connection Warnings (v1.0.87+)" subsection under MCP Servers with a summary of MCP reliability fixes
- `07-github-integration.md`: Added a v1.0.87+ note under Pull Requests Tab Merge Status for the PR badge/GitHub status tabs refresh fix

### Feature Summary (v1.0.87)
- **New:** Auto model routing tier gains user and managed startup defaults, plus a strict/user-overridable organization policy
- **New:** Consecutive steering prompts sent while the agent is busy combine into one pending message, recallable with `Up`; `Ctrl+P` browses history without withdrawing prompts
- **New:** `worktreePathTemplate` setting controls where `/worktree`, `/move`, `/new`, and `--worktree` create worktrees
- **New:** Per-server MCP `slowConnectionThresholdMs` setting
- **Fixed:** Sandbox proxies work on Windows; proxy username/password auth works on every platform; question-dialog number-key selection past 9; `/keep-alive` failure reporting; prompt-mode child-task recovery; large-session resume reliability; `--yolo` persistence after startup policy checks; empty `strictKnownMarketplaces` now hides built-in marketplaces; `/help`/`/mcp show` mouse-selection highlighting; git-status process teardown on timeout; MCP auth-status accuracy during reconnects; PR badge/GitHub status tabs after auth or branch refresh; a failing MCP server no longer removes other servers' tools; `copilot mcp list`/`get` report the built-in `github-mcp-server`; MCP servers with list-change capabilities connect instead of failing; subagent tool-request approval deduplication; secrets no longer written to debug logs; managed plugin marketplace policy loading; MCP reconnect session-resume hang fix; live elapsed time for execution subagents in the timeline; rubber-duck agent enabled for every model family and low-cost-tier session models; reduced terminal repaint allocation overhead
